...
Public Class MainForm
    ...
    '''' <summary>
    '''' 出力フォルダ配下にサブフォルダを作成する
    '''' </summary>
    'Private Sub EnsureFolders(basePath As String)
    '    Dim folders() As String = {"Dto", "Entity", "Model"}
    '    For Each f In folders
    '        Dim path = System.IO.Path.Combine(basePath, f)
    ...
    Private Sub btnGenerate2_Click(sender As Object, e As EventArgs) Handles btnGenerate2.Click
        Try
            ...
            For Each tableName As String In clbTables.CheckedItems
                ...
                dict("ColumnList") = String.Join(", ", cols.Select(Function(c) c.ColumnName))
                dict("ParamList") = String.Join(", ", cols.Select(Function(c) "@" & c.ColumnName))
...
"        If reader(""" & col.ColumnName & """) Is DBNull.Value Then
    ...
    'Public Function LoadTemplate(path As String) As String
    '    Return System.IO.File.ReadAllText(path, System.Text.Encoding.UTF8)
    'End Function
    'Public Function ApplyTemplate(template As String, dict As Dictionary(Of String, String)) As String
    '    Dim result As String = template
    '    For Each key In dict.Keys
    '        result = result.Replace("{" & key & "}", dict(key))
    '    Next
