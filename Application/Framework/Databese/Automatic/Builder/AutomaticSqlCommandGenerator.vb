...
''' </summary>
Public Class AutomaticSqlCommandGenerator
    ...
    ''' GETDATE() または currentUser を自動設定する。
    ''' </summary>
    ''' <param name="dto">UPDATE 対象の DTO 型</param>
    ''' <param name="tableName">UPDATE 対象のテーブル名</param>
    ''' <param name="currentUser">更新者として設定するユーザー名</param>
    ''' <returns>生成された UPDATE SQL 文</returns>
    Public Shared Function GenerateUpdate(Of T)(dto As T, tableName As String, currentUser As String) As String

        Dim props = dto.GetType.GetProperties()
    ...
    ''' <summary>
    ''' DTO のプロパティを元に DELETE 文を自動生成する。
    ''' PrimaryKeyAttribute / RowVersionAttribute が付与されたプロパティは
    ''' WHERE条件に自動設定する。
    ''' </summary>
    ''' <param name="dtoType">DELETE 対象の DTO 型</param>
    ''' <param name="tableName">DELETE 対象のテーブル名</param>
    ''' <param name="currentUser">未使用</param>
    ''' <returns>生成された DELETE SQL 文</returns>
